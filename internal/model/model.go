...
func (rt *Runtime) forwardEmbeddingForSampling(embedding []float32, pos ropePos, caches []kvCache, scratch *generationScratch, needLogits bool, opts GenerateOptions) (generationForwardResult, error) {
	...
	for li := 0; li < c.NumHiddenLayers; li++ {
		...
		if hasRoPE && !lastLayer && tl.q8.q != nil && rt.vulkanOpEnabled(vulkanOpLayerChainQ8) {
			...
			if rt.vulkanLayerChainQ8(layers[li+1].norm[:c.HiddenSize], h, h, tl.w.ln1, &caches[li], tl, ropeCos, ropeSin, tl.w.ln2, c.NumAttentionHeads, c.NumKeyValueHeads, c.HeadDim, nextNormWeight, li > 0, true, true) {
		...
		if hasRoPE && lastLayer && needLogits && tl.q8.q != nil && rt.vulkanOpEnabled(vulkanOpLayerChainQ8) {
			...
			if rt.vulkanLayerChainQ8(scratch.norm[:c.HiddenSize], h, h, tl.w.ln1, &caches[li], tl, ropeCos, ropeSin, tl.w.ln2, c.NumAttentionHeads, c.NumKeyValueHeads, c.HeadDim, finalNormWeight, li > 0, false, false) {
...
func (rt *Runtime) vulkanLayerChainQ8(normOut, residual, rawInput, ln1Weight []float32, cache *kvCache, tl *textLayer, cosTable, sinTable, ln2Weight []float32, numHeads, kvHeads, headDim int, nextNormWeight []float32, devInputReady bool, readResidual bool, skipNormResidualReadback bool) bool {
