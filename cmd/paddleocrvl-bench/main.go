...
func inputIDs(tok *tokenizer.Tokenizer, body requestBody) []int {
	if len(body.Tokens) > 0 {
		return body.Tokens
	}
	if body.Task != "" {
		return tok.EncodeReadOnly(chatPrompt(taskPrompt(body.Task), body.ImagePath != "" || body.ImageBase64 != ""))
	}
	return tok.EncodeReadOnly(body.Prompt)
}
